...
def attention_model_training(data_loaders, text_params, audio_params,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_attention_model(epoch, clip, train_loader, attn_model,
                              criteria_list, criteria_weights, optimizer_list)

        # evaluate performance on  test set
        train_loss, (y_train_pred, y_train_gold) = \
            eval_attention_model(train_loader, attn_model, criteria_list, criteria_weights)
        # evaluate performanve on valid set

        valid_loss, (y_valid_pred, y_valid_gold) = \
            eval_attention_model(valid_loader, attn_model, criteria_list, criteria_weights)
        ...
        test_loss, (y_test_pred, y_test_gold) = \
            eval_attention_model(test_loader, attn_model, criteria_list, criteria_weights)
